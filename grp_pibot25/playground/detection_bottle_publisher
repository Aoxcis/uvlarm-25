...
class Realsense(Node):
    def __init__(self, fps=30):
        super().__init__('realsense_node')  # Nom du nœud
        ...
        try:
            ...
            self.get_logger().info("Pipeline RealSense démarré avec succès.")
        except Exception as e:
            self.get_logger().error(f"Erreur lors du démarrage du pipeline : {e}")
            exit(1)
        ...
        self.lower_green = np.array([50, 100, 50])  # Limite basse pour le vert
        self.upper_green = np.array([70, 255, 255])  # Limite haute pour le vert
    ...
    def read_and_process_frames(self):
        ...
        if not depth_frame or not color_frame:
            self.get_logger().warn("Impossible de récupérer les frames.")
            return None, None, None

        # Obtenir les images couleur et profondeur
    ...
    def process_and_publish(self):
        ...
        for x, y, w, h, distance in detected_objects:
            ...
            message = f"Object detected at ({x}, {y}), Distance: {distance:.2f}m"
            self.detection_publisher.publish(String(data=message))

        # Publier l'image couleur
